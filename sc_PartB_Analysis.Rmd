...
The B.6. module saves an intermediate Seurat and biologic object -->
```{r child = 'src/modules/section_B/B.6.create.pca.plots.Rmd', eval=TRUE}
```

```{r, eval=FALSE, echo=T}

## This optional chunk can be used to load a prexisting OsC and Obio object, so that the time-intense processing steps don't need to be done again. 
## Load OsC and Obio from previous run ##
## default: false
...
```{r child = 'src/modules/section_B/B19.example.feature.plots.Rmd', eval=FALSE}
```
