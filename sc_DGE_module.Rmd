...
for (i in 1:length(DGEtagVec)){
    ...
    colVec <- colVec[names(colVec) %in% unique( dfPlot$DGE_Status)]
...
sheet <- substr(paste0(Obio@parameterList$project_id, "_subcluster_DGE"),1,30)
...
cmd.vec <- upload.datatable.to.database(
    ...
    db.col.parameter.list = list(
        ...
        "DECIMAL(6,3) NULL DEFAULT NULL" = c("Integrated_Samples_","_avg_diff_","Ind_var","add_counts_",  "lg2BaseMean","lg2baseMean","contrast_P_PCA_estimatePCA","^CoVar$","NES", "logFC", "lg2_avg","norm_counts", "contrast_C_corCoef_Inhba_Fib", "intensity", "^int", "iBAQ","^localization_prob$", "stat", "lg10p","contrast_x_", "add_MA_cts_MA"),
