...
if (length(unique(dfPlot$Cluster)) > 15){
   plotList[[tag]] <- plotList[[tag]] + ggplot2::theme(legend.position = "none")
            ...
            link <- paste0(
              ...
              '<a href="https://',urlString,'/',Obio@parameterList$project_id,'/pca?x_axis=UMAP_1&y_axis=UMAP_2" target="_blank">here</a>', '. ')  
...
if (length(unique(dfPlot$Cluster)) > 15){
   plotList[[tag]] <- plotList[[tag]] + ggplot2::theme(legend.position = "none")
            ...
            link <- paste0(
              'An interactive version of this figure can be found ', 
              '<a href="https://',urlString,'/',Obio@parameterList$project_id,'/pca?x_axis=tSNE_1&y_axis=tSNE_2" target="_blank">here</a>', '. '
              )  
