...
# # module purge;source /nemo/stp/babs/working/software/modulepath_new_software_tree_2018-08-13;module load pandoc/2.2.3.2-foss-2016b;ml R/4.1.2-foss-2021b

# newest R-version 
# Create conda environment with R43

# conda activate R422;R

# Run singularity-R
# ml Singularity/3.11.3
# singularity shell --bind  /nemo:/nemo,/camp:/camp /nemo/stp/babs/working/boeings/singularity_images/r431.ubuntu.16.04.sif;mkdir -p ~/.R;echo 'CXX17 = g++-7 -std=gnu++17 -fPIC' > ~/.R/Makevars;

# Fixing rccpToml installation issue
# mkdir -p ~/.R
...
# Check version BiocManager::version()
# BiocManager::install(version = "3.15")


## Make sure all repos are available for bioconductor
...
# Try installing DESeq2
# renv::install("bioc::DESeq2")
# The above fails. "Error: package 'DESeq2' is not available"

# Next attempt:
#BiocManager::install(c('DESeq2'))
# Several https bioconductor repositories appear unavailable.
#options(BioC_mirror = "http://bioconductor.org")
# let's try again:
...
#if (!requireNamespace("biologicSeqTools2")){
#  #BiocManager::install(c('BiocGenerics', 'DelayedArray', 'DelayedMatrixStats','limma', 'lme4', #'S4Vectors', 'SingleCellExperiment', 'SummarizedExperiment', 'batchelor', 'Matrix.utils', #'HDF5Array', 'terra', 'ggrastr'), type = "source")
 ...
 # Sys.setenv(BIOCONDUCTOR_CONFIG_FILE="http://bioconductor.org/config.yaml")
  ...
  ## For now, DESeq2 only installs via this route, 
#  BiocManager::install(c('DESeq2'))
  ## but not using this route:
  # renv::install("bioc::DESeq2")
...
if (!requireNamespace("lemur")){
  renv::install("const-ae/lemur")
}
...
subset4test <- TRUE
if (!exists("subset4test")){
  subset4test <- FALSE
}

if (subset4test){
  set.seed(42)
  n.cells <- round(0.2 * nrow(OsC))
  cells.use <- sample(x= OsC@meta.data$cellID, size=n.cells)
  OsC <- OsC[, colnames(OsC) %in% cells.use]
}
...
# sce <- Seurat::as.SingleCellExperiment(OsC, assay = "RNA")

cells.use <- OsC@meta.data[OsC@meta.data$meta_Treatment %in% c("Control", "MatB52"), "cellID"]
OsC <- OsC[, colnames(OsC) %in% cells.use]

rs <- rowSums( data.matrix(OsC@assays[["RNA"]]@counts))
OsC <- OsC[rowSums( data.matrix(OsC@assays[["RNA"]]@counts)) > 0, ]

# create single-cell experiment manually
count_matrix <- data.matrix(OsC@assays[["RNA"]]@counts)
count_matrix <- count_matrix[rowSums(count_matrix) > 0,]

cell_metadata <- data.frame(OsC@meta.data)
cell_metadata <- cell_metadata[colnames(count_matrix),]
...
## Test
fit <- lemur::lemur(sce, design = ~  meta_Replicate + meta_Treatment , n_embedding = 15)
fit <- lemur::align_harmony(fit)   # This step is optional
fit <- lemur::test_de(fit, contrast = cond(meta_Treatment =  "MatB52") - cond(meta_Treatment =  "Regeneron"))
fit <- lemur::test_de(fit, contrast = cond(meta_Treatment =  "MatB52") - cond(meta_Treatment =  "Control"))
...
fit <- lemur::test_de(fit, contrast = cond(meta_Treatment =  "Regeneron") - cond(meta_Treatment =  "Control"))

umap <- uwot::umap(t(fit$embedding))

p1 <- as_tibble(fit$colData) %>%
  mutate(umap = umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = meta_Treatment, shape = meta_Replicate), size = 0.5) +
...
as_tibble(fit$colData) %>%
  mutate(umap = umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = clusterName, shape = meta_Replicate), size = 0.5) +
    labs(title = "UMAP of latent space from LEMUR")


nei <- lemur::find_de_neighborhoods(fit, group_by = vars(meta_Replicate, meta_Treatment))

dfRC <- data.frame(
  as_tibble(nei) %>%
    arrange(pval) %>%
    dplyr::select(name, n_cells, pval, adj_pval, lfc)
)

sel_gene <- "Mafb"

pg1 <- tibble(umap = umap) %>%
...
p2 <- as_tibble(fit$colData) %>%
  mutate(umap = umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = meta_Treatment, shape = meta_Replicate), size = 0.5) +
    labs(title = "UMAP of latent space from LEMUR")

pg2 <- tibble(umap = umap) %>%
  mutate(expr = assay(fit, "DE")[sel_gene,]) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = expr)) +
    scale_color_gradient2() +
...
p3 <- as_tibble(fit$colData) %>%
  mutate(umap = umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = meta_Treatment, shape = meta_Replicate), size = 0.5) +
    labs(title = "UMAP of latent space from LEMUR")

pg3 <- tibble(umap = umap) %>%
  mutate(expr = assay(fit, "DE")[sel_gene,]) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = expr)) +
    scale_color_gradient2() +
...
orig_umap <- uwot::umap(as.matrix(t(SingleCellExperiment::logcounts(sce))))

library(ggplot2)

as_tibble(SingleCellExperiment::colData(sce)) %>%
...
nei[["label"]] <- ""
nei[nei$adj_pval < 0.1, "label"] <- nei[nei$adj_pval < 0.1, "name"]

nei %>%
  ggplot(aes(x = n_cells, y = -log10(pval)), label=label) +
    geom_point(aes(color  = adj_pval < 0.1, label=label)) +
    labs(title = "Neighborhood size vs neighborhood significance") +
  geom_text(aes(label=label))

nei[["label"]] <- ""
nei[nei$name == "Inhba", "label"] <- nei[nei$name == "Inhba", "name"]
...
## Works with Phase, but not meta_Treatment

fit <- lemur::lemur(sce, design = ~  sampleName +Phase , n_embedding = 15)
fit <- lemur::align_harmony(fit)   # This step is optional
fit <- lemur::test_de(fit, contrast = cond(Phase =  "G1") - cond(Phase = "G2M"))
nei <- lemur::find_de_neighborhoods(fit, group_by = vars(sampleName, Phase))

## Create latent space UMAP
umap <- uwot::umap(t(fit$embedding))

as_tibble(fit$colData) %>%
  mutate(umap = umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = sampleID, shape = Phase), size = 0.5) +
    labs(title = "UMAP of latent space from LEMUR")
...
###############################################################################
##
fit <- lemur::lemur(sce, design = ~ Phase , n_embedding = 15)
fit <- lemur::align_harmony(fit)   # This step is optional
fit <- lemur::test_de(fit, contrast = cond(Phase =  "G1") - cond(Phase = "G2M"))
nei <- lemur::find_de_neighborhoods(fit, group_by = vars(Phase))
...
## Test only
#OsC@meta.data[["test"]] <- OsC@meta.data[["sampleName"]]

segmentationCol <- "meta_Timepoint"

## Add column for averaging of cells
selClust <- paste0("clusterName_", segmentationCol)
OsC@meta.data[[selClust]] <- paste0(
  OsC@meta.data$clusterName,
  "_",
  OsC@meta.data[,segmentationCol]
)


if (segmentationCol == "sampleName" & length(grep("^sampleColor$", names(OsC@meta.data)) == 1)){
  ...
  allCols <- unique(OsC@meta.data[,segmentationCol])
  colorSel <- scales::hue_pal()(length(allCols))
  
  OsC@meta.data[[segmentationColorCol]] <- OsC@meta.data[[segmentationCol]]
  for (i in 1:length(allCols)){
    OsC@meta.data[[segmentationColorCol]] <- gsub(
      allCols[i],
      colorSel[i],
      OsC@meta.data[[segmentationColorCol]]
...
for (i in 1:length(LRToptions)){
    ...
    name <- paste0("LRT_", as.vector(dfTemp[i, "clusterName"]), "_", segmentationCol)
...
for (i in 1:length(LRTtagVec)){
  ...
  openxlsx::addFilter(wb, sheetName, row = 1, 1:ncol(dfOutput))
...
for (i in 1:length(LRTtagVec)){
    ...
    plotList[[tag]] <- ggplot2::ggplot(
      ...
      xLabel <- names(dfPlot[grep("_cts_", names(dfPlot))])      
...
dfAdd <- unique(OsC_HM@meta.data[,c("clusterName", "clusterColor", segmentationCol,  segmentationColorCol, selClust)])
...
# )
# Obio@parameterList$sampleNameOrder <- c(
#    "E185",
#    "PD4",
#    "PD7"
# )
...
if (segmentationCol == "sampleName" & !is.null(Obio@parameterList$sampleNameOrder)){
...
} else if (segmentationCol == "meta_Timepoint"){
  segmentationColOrder <- c(
      "T0hrs", 
      "T2hrs", 
      "T4hrs",
      "T8hrs",
  ...
  segmentationColOrder <- unique(OsC@meta.data[,segmentationCol])
}
