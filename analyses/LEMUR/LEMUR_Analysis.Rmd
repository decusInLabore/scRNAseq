...
sce <- Seurat::as.SingleCellExperiment(OsC)
...
## It seems that there can't be co-linearity between the variables. Some 
## differences are required. 

fit <- lemur::lemur(sce, design = ~ meta_Gender + meta_Tumor_Normal, n_embedding = 15)
fit <- lemur::align_harmony(fit)   # This step is optional
fit <- lemur::test_de(fit, contrast = cond(meta_Tumor_Normal =  "Tumor") - cond(meta_Tumor_Normal = "Normal"))
nei <- lemur::find_de_neighborhoods(fit, group_by = vars(meta_Gender, meta_Tumor_Normal), include_complement =F)
# This doesn't work
...
sceRef <- lemur::glioblastoma_example_data
###############################################################################
## Plot                                                                      ##
set.seed(42)
...
as_tibble(SingleCellExperiment::colData(sce)) %>%
  ...
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = meta_Gender, shape = meta_Tumor_Normal), size = 0.5) +
    labs(title = "UMAP of logcounts")

as_tibble(SingleCellExperiment::colData(sce)) %>%
  mutate(umap = orig_umap) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    geom_point(aes(color = meta_Tumor_Normal, shape = meta_Gender), size = 0.5) +
    labs(title = "UMAP of logcounts")
