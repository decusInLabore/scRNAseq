...
# Obio@dataTableList$dfGeneralMarkers
...
for (i in 1:length(clusterVec)){
    ...
    ) + ggrastr::rasterize(ggplot2::geom_point( shape=16, size = dotsize
            ), dpi = rasterDpi)  + xlab("Average Expression All Cells") + ylab(paste0("Average Expression ", clusterVec[[i]])
            ) + theme_bw(
            )  +  theme(
