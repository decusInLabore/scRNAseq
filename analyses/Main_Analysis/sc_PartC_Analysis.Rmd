...
workdir <- gsub("scripts/scRNAseq/analyses/Main_Analysis","workdir/",getwd())
...
addFNvec <- c(
  paste0(workdir, "temp/UMAP.regression.coordinates.txt"),
  paste0(workdir, "temp/ClusterFinder.output.txt"),
  paste0(workdir, "temp/species.inference.txt"),
  paste0(workdir, "temp/integration.trials.txt")
)
...
## Define helper function ##
perc_fun <- function(obj, genes, assay = "RNA") {
  z <- subset(obj, features = genes)[[assay]]@data
  
  z[z > 0] <- 1  
  ExprMatrix <- round(( rowSums(z) / ncol(z) ) * 100, 1 )
  dfExprMatrix <- data.frame(ExprMatrix)
  dfExprMatrix[["gene"]] <- row.names(dfExprMatrix)
  colnames( dfExprMatrix )[1] <- "count_cut_off"
  return(dfExprMatrix)
}

#####################################################
## Calculate perc expression for all genes         ##

allGenes <- row.names(OsC)
geneList <- split(allGenes, ceiling(seq_along(allGenes)/500))

## To speed up calculations, large datasets 

dfPercCellsExpr <- purrr::map(geneList, function(x) perc_fun(obj=OsC, genes = x)) %>%
    dplyr::bind_rows() %>%
    data.frame()
...
if (Obio@dbDetailList$host == "10.27.241.234"){
    shinyBasePath <- "/camp/stp/babs/www/shiny/internal/users/boeings/"
    ...
    if (dir.exists("/camp/stp/babs/www/shiny/external/users/boeings/")){
        shinyBasePath <- "/camp/stp/babs/www/shiny/external/users/boeings/"  
    } else {
        shinyBasePath <- "../"
    }
...
## Change colors to previous project colors ##
dfCol1 <- unique(biologicSeqTools2::import.db.table.from.db(
    dbname = "pbl_data",
    dbtable = "pbl359subCluster07_PCA",
    host =  Obio@dbDetailList$host,
    user =  Obio@dbDetailList$db.user,
    password = db.pwd
)[,c("clusterName", "clusterColor")] )

dfCol1$clusterName <- paste0(
    "org_cluster_",
    dfCol1$clusterName
)

for (i in 1:nrow(dfCol1)){
    params$catColorList$original_clusterName[dfCol1[i, "clusterName"]] <- dfCol1[i, "clusterColor"]
}

dfCol2 <- unique(biologicSeqTools2::import.db.table.from.db(
    dbname = "pbl_data",
    dbtable = "SC22096_PCA",
    host =  Obio@dbDetailList$host,
    user =  Obio@dbDetailList$db.user,
...
dfCol2$clusterName <- paste0(
    "org_cluster_",
    dfCol2$clusterName
)

for (i in 1:nrow(dfCol2)){
    params$catColorList$original_clusterName[dfCol2[i, "clusterName"]] <- dfCol2[i, "clusterColor"]
}

#params$catColorList$original_clusterName
...
createHTMLoutput <- function(FvFN = NULL, Obio = NULL){
    ...
    cat('</head>');cat("\n");
    cat('<body>');cat("\n");
    cat(paste0('<iframe src="https://bioinformatics.crick.ac.uk/shiny/users/boeings/',Obio@parameterList$project_id,'_app/" title="',Obio@parameterList$labname, ' Lab"></iframe>'));cat("\n");
    cat('</body>');cat("\n");
    cat('</html>');cat("\n");
    sink()
