...
```{r child = '08-euclidian.distances.figure.Rmd', eval=FALSE}
```


<!-- Optional 10: (Output only): Doublet Estimate Chapter/Module -->
```{r child = '09-doublet.estimation.plots.Rmd', eval=FALSE}
```
...
```{r child = '13-linear.regression.data.prep.Rmd', eval=FALSE}
```
...
```{r child = '16-gene.set.heatmaps.Rmd', eval=FALSE}
```


<!-- Essential 19: (output required later):Category Make cluster table Data Prep Chapter/Module -->
```{r child = '17-example.feature.plots.Rmd', eval=FALSE}
```
