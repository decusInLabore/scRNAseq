...
### DGE Instructions from Roberta:
To address cTEC differences, we would need to perform
pbl359B 0.7 DEGS of each of the following clusters in all samples against each other C0,3,6,7

Action taken based on the above instruction:
DGE C0 vs C3
DGE C0 vs C6
DGE C0 vs C7
DGE C3 vs C6
...
To address neuroendocrine TEC differences, we would need to perform 
pbl359B 0.7 DEGS of each of the following clusters in all samples
C1 vs C4 (neuro I vs II)
C2 vs C11 (neuro III vs IV)
...
for (i in 1:length(DGEtagVec)){
  ...
  openxlsx::addFilter(wb, i, row = 1, cols = 1:ncol(dfOutput))
...
for (j in 1:length(DGEtagVec)){
    ...
    if ((length(posTestGeneSet) >= 3) | (length(negTestGeneSet) >= 3)){
        ...
        sheetName <- paste0(substr(DGEtagVec[i],1,23), "_comp_",i)

        openxlsx::addWorksheet(wb, sheetName)
        openxlsx::freezePane(wb, sheetName ,  firstActiveRow = 2)
        openxlsx::writeData(wb, 1, dfResTable, startRow = 1, startCol = 1, headerStyle = hs1)
        openxlsx::addFilter(wb, 1, row=1, cols = 1:ncol(dfResTable))
