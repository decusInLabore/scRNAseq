...
OsC@meta.data[["seurat_clusters_meta_Tumor_Normal"]] <- paste0(
    "C",
    OsC@meta.data$seurat_clusters,
    "_",
    OsC@meta.data$meta_Tumor_Normal
)



dfTemp <- unique(OsC@meta.data[,c("clusterName", "seurat_clusters", "seurat_clusters_meta_Tumor_Normal", "meta_Tumor_Normal")])
...
groupCol <- "seurat_clusters_meta_Tumor_Normal"
...
DGEinputList[["comp_3"]]  = list(
      "name"    = "DGE_iCAFs_vs_myCAFs",
...
orderVec <- c( "comp_1", "comp_2", "comp_3", "comp_4" )
DGEinputList <- DGEinputList[orderVec]
...
OsC_HM@meta.data[[selClust]] <- paste0(
  ...
  "_",
  OsC_HM@meta.data$meta_Tumor_Normal
)
...
dfAdd <- unique(OsC_HM@meta.data[,c("clusterName", "clusterColor",  "meta_Tumor_Normal", selClust)])

dfAdd[["diseaseGroupColor"]] <- dfAdd$meta_Tumor_Normal
dfAdd$diseaseGroupColor <- gsub("Normal", "#0000ff", dfAdd$diseaseGroupColor)
dfAdd$diseaseGroupColor <- gsub("Tumor", "#ff0000", dfAdd$diseaseGroupColor)
...
OsCsel@meta.data$meta_Tumor_Normal <- factor(
  OsCsel@meta.data$meta_Tumor_Normal,
...
OsCsel@meta.data <- OsCsel@meta.data[order(OsCsel@meta.data$clusterName,OsCsel@meta.data$meta_Tumor_Normal),]
...
for (i in 1:length(HMgeneSelections)){
  if (length(unlist(HMgeneSelections[[i]])) > 1){
    returnList <- addHeatmap2List(
        ...
        group1ColorCol = "clusterColor", 
        group1NameCol = "clusterName",
        group2ColorCol = "diseaseGroupColor", 
        group2NameCol = "meta_Tumor_Normal",
        chnkVec = chnkVec,
        columnSplitCol = "clusterName"
