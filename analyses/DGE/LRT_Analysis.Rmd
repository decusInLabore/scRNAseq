...
DGEoptions <- unique(dfTemp[,groupCol])
...
for (i in 1:length(LRTtagVec)){
  ...
  if (!is.null(Obio@dataTableList[["referenceList"]]$GO_Secreted)){
    cat <- Obio@dataTableList[["referenceList"]]$GO_Secreted
...
for (i in 1:length(LRTtagVec)){
    ...
    plotList[[tag]] <- ggplot2::ggplot(
        ...
        ggplot2::geom_hline(yintercept = c(2), col = "#000000", lty=2) +   
        ggplot2::geom_point( shape=16, size = as.numeric(dotsize),alpha = 0.7, color="darkgrey") + 
...
for (i in 1:length(LRTtagVec)){
    tag <- paste0("MA_Plot_", LRTtagVec[i])  
...
for (j in 1:length(LRTtagVec)){
    ...
    negTestGeneSet <- as.vector(
        unique(
            EnrichedGenesList[[paste0(LRTtagVec[j], "_neg")]]
        )
    )
    ...
    if ((length(posTestGeneSet) >= 3) | (length(negTestGeneSet) >= 3)){
        ...
        if (length(posTestGeneSet) >= 3){
            ...
            ## Save to file ##
            FNbase <- paste0("DGE_comparison_", LRTtagVec[j],".enriched.genes", VersionPdfExt)
            FN <- paste0(Obio@parameterList$reportFigDir, FNbase)
            FNrel <- paste0("report_figures/", FNbase)
