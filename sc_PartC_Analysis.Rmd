...
## For now: Downsample to 60k cells
if (nrow(OsC) > 60000){
    OsC <- OsC[, sample(colnames(OsC), size = 60000, replace=F)]
}
...
## Create cluster by Tumor_Normal
...
## Make switch to ExprPerCluster when clusterbysample is very large
maxNormCountsCutOff <- 300
if (ncol(dfNormCounts) > maxNormCountsCutOff){
    dfNormCounts <- Obio@dataTableList$dfAvglg10ExprPerCluster
}
...
if (ncol(dflg2Avg) > maxNormCountsCutOff){
    dflg2Avg <- Obio@dataTableList$dfAvglg10ExprPerClusterScaled
}
...
## Dealing with long big tables: ERNO139 can appear in large tables well before 
## the 4096 column limit (...). To avoid this, the cluster-sample average columns 
## will not be added to big datasets. 
