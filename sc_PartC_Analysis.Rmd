...
biologicSeqTools::upload.datatable.to.database(
    ...
    df.data = dfdbTable,
    db.col.parameter.list = biologicSeqTools::inferDBcategories(dfdbTable),
    new.table = TRUE
)
...
###############################################################################
## Add spliced lg10 expr values                                              ##
## Add spliced counts to assays
###############################################################################
## Function source                                                           ##
...
## remove all gene names that are not in the experiment ##
genesInExperiment <- unique(row.names(OsC))
unityMatSpliced <- unityMatSpliced[row.names(unityMatSpliced) %in% genesInExperiment,]
unityMatUnSpliced <- unityMatUnSpliced[row.names(unityMatUnSpliced) %in% genesInExperiment,]
...
dfExpr <- dfExpr[dfExpr$expr != 0,]
dfExpr[["gene"]] <- paste0(dfExpr$gene, "_Spl")
...
Obio@dataTableList$dfResS <- dfExpr

FN <- "../temp/lg10expr.spliced.txt"
write.table(dfExpr, FN, row.names = F, sep="/t")



mUS <- unityMatUnSpliced
mUS <- mUS + 1 
mUS <- apply(mUS, 2, log10)

dfExpr <- data.frame(mUS)
dfExpr[["gene"]] <- row.names(dfExpr)
dfExpr <- gather(
    dfExpr, 
    condition, 
...
dfExpr <- dfExpr[dfExpr$expr != 0,]
dfExpr[["gene"]] <- paste0(dfExpr$gene, "_UnSpl")

names(dfExpr) <- gsub("condition", "cellID", names(dfExpr))
names(dfExpr) <- gsub("expr", "lg10Expr", names(dfExpr))
dfExpr$lg10Expr <- round(dfExpr$lg10Expr, 3)

Obio@dataTableList$dfResUS <- dfExpr

FN <- "../temp/lg10expr.unspliced.txt"
write.table(dfExpr, FN, row.names = F, sep="/t")
...
dfExpr <- dfExpr[dfExpr$expr != 0,]
dfExpr[["gene"]] <- paste0(dfExpr$gene, "_SdUS")

names(dfExpr) <- gsub("condition", "cellID", names(dfExpr))
names(dfExpr) <- gsub("expr", "lg10Expr", names(dfExpr))
dfExpr$lg10Expr <- round(dfExpr$lg10Expr, 3)

Obio@dataTableList$dfResSdUS <- dfExpr

FN <- "../temp/lg10expr.s.minus.us.txt"
write.table(dfExpr, FN, row.names = F, sep="/t")
...
# Spliced #
OsC[["spliced"]] <- CreateAssayObject(counts = unityMatSpliced )
OsC[["unspliced"]] <- CreateAssayObject(counts = unityMatUnSpliced )


###############################################################################
## Add Categories                                                            ##

pos <- grep("^dfResS$", names(Obio@dataTableList))

if (length(pos) > 0) {
    dfCat <- Obio@dataTableList$dfResS
 
    names(dfCat) <- gsub("cellID", "condition", names(dfCat))
    names(dfCat) <- gsub("lg10Expr", "expr", names(dfCat))
    dfCat <- dfCat[dfCat$expr != 0,]
    dfCat <- dfCat[,names(dfExpr)]
    dfExpr <- rbind(dfExpr, dfCat)
}

## Done adding S counts                                                    ##
###############################################################################

###############################################################################
## Add Categories                                                            ##

pos <- grep("^dfResUS$", names(Obio@dataTableList))

if (length(pos) > 0) {
    dfCat <- Obio@dataTableList$dfResUS
...
## Done adding US counts                                                    ##
###############################################################################

###############################################################################
## Add Categories                                                            ##

pos <- grep("^dfResSdUS$", names(Obio@dataTableList))

if (length(pos) > 0) {
    dfCat <- Obio@dataTableList$dfResSdUS
